      ...
      <CardHeader className="pb-3 px-4 pt-4">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
          <div className="flex-1 min-w-0">
            <CardTitle className="text-base sm:text-lg font-semibold text-gray-800 truncate">{order.id}</CardTitle>
            <p className="text-sm text-gray-600 mt-1 truncate">{order.customerName}</p>
          </div>
          <Badge className={`${getStatusColor(order.status)} flex items-center gap-1 text-xs whitespace-nowrap`}>
            {getStatusIcon(order.status)}
            <span className="hidden sm:inline">{getStatusDisplayName(order.status)}</span>
            <span className="sm:hidden">{status === 'out-for-delivery' ? 'Out' : getStatusDisplayName(order.status)}</span>
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="px-4 pb-4">
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold text-green-600 text-base">${order.total.toFixed(2)}</span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Time</span>
            <span className="text-xs sm:text-sm text-right">{order.orderTime}</span>
          </div>
          ...
          <div className="flex flex-col gap-2 mt-4">
            <Dialog open={showDetails} onOpenChange={setShowDetails}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="w-full h-10 touch-manipulation">
                  ...
                  <div className="border-b pb-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium min-w-0">Name:</span>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <span className="text-sm leading-relaxed">{order.deliveryAddress}</span>
                      </div>
                    </div>
                  </div>
